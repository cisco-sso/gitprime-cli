...
.PHONY: gen
gen: client/gitprime_cli_client.go  # Regenerate generated artifacts.
client/gitprime_cli_client.go: api/swagger/swagger.json
	./bin/swagger generate client -f ./api/swagger/swagger.json --name $(PROGRAM)
...
.PHONY: tags
tags:  # Create tags for emacs
tags:
	rm -f TAGS && etags -a $$(find * -type f -name '*.go')
